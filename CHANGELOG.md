# Changelog

Alle nennenswerten Änderungen an diesem Projekt werden in dieser Datei dokumentiert.

Das Format basiert auf [Keep a Changelog](https://keepachangelog.com/de/1.0.0/),
und dieses Projekt folgt [Semantic Versioning](https://semver.org/lang/de/).

---

## [Unreleased]

### Planned
- Fish-Config Basis (shared/usr/local/share/fish/)
- Docs-Struktur (shared/home/mrohwer/Documents/)
- Submodules: n8-vps, n8-kiste

---

## [0.2.0] - 2026-01-23

### Added
- Secrets-Management mit Age-Encryption
- SSH-Key für Codeberg (id_ed25519_codeberg)
- API-Token für Codeberg (codeberg_api.token)
- derive_key.fish für Master-Password Derivation
- symlinks.db für automatisches Deployment
- SECRETS.md Dokumentation

### Changed
- .gitignore angepasst für Age-verschlüsselte Secrets
- README.md mit aktuellem Status

---

## [0.1.0] - 2026-01-22

### Added
- Repository initialisiert in `/mr-bytez`
- Basis-Ordnerstruktur (shared/, projects/, .claude/, .config/)
- Root-Dateien (README, CHANGELOG, ROADMAP, .gitignore)
- Git-Konfiguration (main branch)
- Codeberg Repository erstellt (n8lauscher/mr-bytez)

---

**Letzte Aktualisierung:** 2026-01-23
